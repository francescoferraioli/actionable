...
Guidance for coding agents working on Actionable, an Electron + React + TypeScript + SQLite accountability todo app. Read the [README](README.md) for the product concepts (todo definitions, schedules, actions, events) before changing behaviour.
...
- **Every action state change appends an event.** Complete, dismiss, snooze and reopen must go through [action-service.ts](src/main/services/action-service.ts), which writes to `action_events`. Do not update action status directly from a handler.
- **After a mutation, notify.** IPC handlers call `onTodosChanged` (also re-plans the scheduler) or `onActionsChanged`; both refresh the unread badge and broadcast `data-changed`, which the renderer listens to via `useDataVersion`. A mutation that skips this leaves stale UI and a stale badge.
...
- The app takes a single-instance lock keyed on the userData directory. A second instance against the same userData silently exits. Unpackaged runs (dev, e2e without an override) append `-dev` to userData, so dev and the installed app have separate databases and can run simultaneously; `ACTIONABLE_USER_DATA` overrides both.
...
- The unique index on `actions(schedule_id, scheduled_at)` is the dedupe mechanism for schedule fires; creation goes through `createFromSchedule`, which checks it and returns null on duplicates. File-sourced actions use `createFromFile` and have no todo row (`source = 'file'`).

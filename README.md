...
- **Occurrence**: created when a schedule fires. This is the thing you action. It is `pending` until you complete, dismiss or snooze it.
- **Occurrence events**: every action (created, completed, dismissed, snoozed, reopened) is appended to an event-sourcing table, which powers history and analytics.

## Features

- **Inbox**: pending occurrences grouped by day, with Complete, Dismiss and Snooze actions. Dismissing requires picking a reason (the reason list is a seeded, extensible table). Snoozing offers 5 minutes, 15 minutes, 1 hour or a custom duration; when the snooze elapses the occurrence returns to the inbox and you are notified again.
- **Unread state**: like email. Any pending occurrence marks the app unread. The count shows on the macOS dock badge, the menu bar (tray) item, and the in-app inbox badge.
- **Desktop notifications**: an OS notification fires when an occurrence is created or comes back from a snooze. Clicking it opens the app.
- **Background scheduling**: closing the window only hides it. The scheduler keeps running in the main process until you explicitly quit from the tray menu. On startup, the most recent fire missed while the app was not running (within 24 hours) is surfaced as a late occurrence, so commitments do not silently vanish.
- **History**: past occurrences with completion time, dismissal reason or snooze target, filterable by date range, todo, category and status.
- **Analytics**: per-todo completion and dismiss rates, best performing time of day ("You're most consistent with exercise at 7am"), a daily completed/dismissed trend chart, snooze counts and a dismissal reason breakdown, over a 7, 30 or 90 day range.
...
- [Database layer](src/main/db/): `node:sqlite` with [versioned migrations](src/main/db/migrations.ts) and one repository per table ([todos](src/main/db/todo-repository.ts), [schedules](src/main/db/schedule-repository.ts), [occurrences](src/main/db/occurrence-repository.ts), [events](src/main/db/event-repository.ts), [dismiss reasons](src/main/db/dismiss-reason-repository.ts)).
- [Services](src/main/services/): [scheduler](src/main/services/scheduler-service.ts) (next-fire tracking, catch-up, snooze wake), [occurrence actions](src/main/services/occurrence-service.ts) (state transitions plus event sourcing), [todo CRUD](src/main/services/todo-service.ts), [notifications](src/main/services/notification-service.ts), [unread badge and tray](src/main/services/unread-service.ts), [analytics](src/main/services/analytics-service.ts).
...
- [Renderer](src/renderer/src/): React views for [Inbox](src/renderer/src/views/InboxView.tsx), [Todos](src/renderer/src/views/TodosView.tsx), [History](src/renderer/src/views/HistoryView.tsx) and [Analytics](src/renderer/src/views/AnalyticsView.tsx).
...
The event-sourced occurrence log and pure analytics layer are designed so AI recommendations, habit optimisation, suggested schedule changes, weekly reviews, adaptive scheduling and natural language todo creation can be added without reworking the data model.
